...
| `semantic_model` | 25 | Basic declarations, function parameters, nested-scope shadowing, all four import forms and their source/imported/local fields, a parameter and a local redeclaration each shadowing a same-named import, object/array destructuring (including a computed key as a reference), arrow parameters, block scope, catch scope, a switch statement's cases sharing one block scope, `var` hoisting out of a nested block, a `let` scoped to a `for` loop head, the scope parent-chain hierarchy; class/static/object method and setter parameters bound, a method parameter shadowing an outer binding, a getter body owning its own scope, computed member names resolving in the enclosing scope, assignment targets resolving through `resolve_assignment` under shadowing |
