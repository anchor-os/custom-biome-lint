...
| `semantic_model` | 39 | Basic declarations, function parameters, nested-scope shadowing, all four import forms and their source/imported/local fields, a parameter and a local redeclaration each shadowing a same-named import, object/array destructuring (including a computed key as a reference), arrow parameters, block scope, catch scope, a switch statement's cases sharing one block scope, `var` hoisting out of a nested block, a `let` scoped to a `for` loop head, the scope parent-chain hierarchy; class/static/object method and setter parameters bound, a method parameter shadowing an outer binding, a getter body owning its own scope, computed member names resolving in the enclosing scope, constructor parameters (plain/destructured/defaulted/rest) bound and shadowing an outer binding, assignment targets resolving through `resolve_assignment` under shadowing; `for-of`/`for-in` heads binding into a Loop scope, a loop head expression not seeing the loop variable, `var` hoisting out of loop and catch scopes, a named function expression's name visible only inside itself, parameter defaults and nested-arrow defaults, rest parameters and rest destructuring, all five arrow parameter shapes, callable-member bodies not leaking outward, one member's parameter invisible to a sibling, an arrow inside a method resolving to the method's parameter, assignment targets resolving inside methods/constructors/setters/loops/catches, and the class-expression name gap pinned |
...
| `destructure_param_prop_assign` | 10 | Property mutation of a destructured parameter reported; depth 1/2/3 all reported and all anchored on the parameter name; plain parameters, reads, mutating method calls and aliased mutation not reported; mutation inside class/static/object methods, a setter, a class-field arrow and a constructor all reported; a parenthesized chain anchored at the parameter rather than the paren; suppressions work; fixture counts pinned |
...
| `deep_param_prop_assign` | 7 | Depth-2+ chains reported with the chain quoted in the message; depth 1 and destructured roots not reported; arrow-parens style irrelevant; constructor parameters including a rest parameter reported; suppressions work; fixture counts pinned |
