...
//! config banned `DoWhileStatement` — alongside `ForStatement` and
//! `WhileStatement` — as a "no loops, use functional iteration" house style.
...
impl Rule for NoDoWhileStatement {
    ...
    fn description(&self) -> &'static str {
        "Disallow `do...while` loop statements"
    }
    ...
    fn check(&self, file: &FileContext) -> Vec<Violation> {
        ...
        for node in file.tree().descendants() {
            let Some(stmt) = JsDoWhileStatement::cast_ref(&node) else {
                continue;
            };
